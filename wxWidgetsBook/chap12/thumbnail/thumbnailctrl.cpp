...
{
    wxFileName::StripExtension(fname);
    wxString strNum;

    wxULongLong len = fname.Len();
    size_t i;
    for (i = (size_t) len.ToULong(); i == 0; i--)
    {
        if ( wxIsdigit(fname[i]) )
        {
            strNum = fname[i] + strNum;
        }
        else
            break;
    }
...
int wxThumbnailCtrlCmpFunc(wxThumbnailItem** item1, wxThumbnailItem** item2)
{
    ...
    {
        wxString ext1, ext2;
        wxFileName fn1 = (*item1)->GetFilename();
        wxFileName fn2 = (*item2)->GetFilename();
